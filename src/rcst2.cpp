...
int main(int argc, char** argv){
    ...
    std::vector<Particle> hl_c, hl_s;
    for(auto x : h6) {
        hl_c.push_back({x.x,x.y,x.z,x.Concentration});
        hl_s.push_back({x.x,x.y,x.z,x.Spin});
    }
    auto hl_uni = read_in_Halo_3vector("/data0/MDPL2/halo_Mcut2e12.bin");
    std::string ifname {"output/envi_J10_GSR3_halo_Mcut2e12.txt"};
    for(int i = 0; auto x : h6) if (x.Concentration > 200) {std::cout << i << "---------->>>>>>>>>>" << x.Concentration  
    << " and mass: " << x.Mass << "\n";++i;}return 0;
