...
double* fourier_mode_correlation_diff_rlz
...
int main(){
    ...
    force_base_type(0,1);
    auto r0 = fourier_mode_correlation_diff_rlz(dm, hl);
    for(size_t i = 0; i < (GridLen/2 + 1) * GridLen * GridLen; ++i) ofs0 << r0[i] << " ";delete[] r0;
    force_base_type(1,10);
    auto r1 = fourier_mode_correlation_diff_rlz(dm, hl);
...
// cross-correlation-coefficiens r(k) of different realization
double* fourier_mode_correlation_diff_rlz
(std::vector<std::vector<Particle>>& dm,std::vector<std::vector<Particle>>& hl)
{
